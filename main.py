...
def chart_data(ent, select_date = None):
    ...
    ent_df = ent_df.drop(['시가', '고가', '저가', '거래량'], axis=1)    # 시가 고가 저가 거래량 column drop
    ent_df.columns = ['Date', 'Close', 'Low', 'Amount']            # 원래 date, close 만 쓰려 했는데 거래대금 등락률이 추가 된듯
    ent_df['Date'] = ent_df['Date'].astype('str')
    ent_dict = ent_df.to_dict()                                    # ent_dict : date를 key로 설정한 dictionary

    dfcp = ent_df.tail(2)                                          # tail 2 : 가장 최근 2개
    rate_color = dfcp['Close'].values.tolist()                    
    ent_dict['eve'] = rate_color[0]                                # 어제 종가
    ent_dict['today'] = rate_color[1]                              # 오늘 종가
